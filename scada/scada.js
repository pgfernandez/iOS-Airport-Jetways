...
server.get({path : PATH +'/jetways/:jetway' , version : '0.0.1'} , requestJetwayStatus);
...
function updateValue(req, res , next){
  ...
  scadaJetways.update({jetway:req.params.jetway},{$set:fieldToUpdate}, function(error , success){
          if(success){
            ...
            console.log('Updating jetway data fails '+ err);
...
function requestJetwayStatus(req, res , next){
    res.setHeader('Access-Control-Allow-Origin','*');
    console.log('recibido el jetway:' + req.params.jetway);
    scadaJetways.findOne({jetway:req.params.jetway} , function(err , success){
        console.log('Response success '+success);
        console.log('Response error '+err);
        if(success){
            res.send(200 , success);
